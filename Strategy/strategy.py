...
class Strategy(ABC):
    ...
    def _get_highest_order_metrics(self, matches: list, check_favourite: bool = False, highest_type: str = "highest_order"):
        order = self.highest_order_getter(highest_type=highest_type, matches=matches)
        if not order:
            return None, None
    ...
    def highest_order_getter(self, highest_type: str, matches: list) -> dict:
        ...
        return max(
            ...
            max(matches, key=lambda x: x["liquidity_difference"])
        )
